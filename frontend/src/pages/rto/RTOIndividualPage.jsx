...
export default function RTOIndividualPage() {
    ...
    <div style={{ minHeight: '100vh', background: COLORS.bgPage, padding: '32px 24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 22 }}>
        ...
        <button
          ...
          style={{
            background: 'none',
            border: hover ? `2px solid ${COLORS.danger}` : `2px solid ${COLORS.border}`,
            borderRadius: RADIUS.md, padding: '9px 18px', cursor: 'pointer',
            fontSize: 13, fontFamily: "'DM Sans', sans-serif", transition: 'all 0.2s',
            color: hover ? COLORS.danger : COLORS.gray,
          }}
        >
          ← Logout
        </button>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: COLORS.gray, marginTop: 4, marginBottom: 0 }}>
                ...
                <code style={{ background: COLORS.bgPage, padding: '1px 6px', borderRadius: 4, fontFamily: 'monospace' }}>
                  CnNo, RefNo, RTO_RECEIPT_DATE, RTO_DELIVERY_DATE, RECEIVER_NAME
                </code>
              </p>
            </div>
            {csvRows.length > 0 && (
          ...
          {!csvRows.length && (
            ...
            >
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: COLORS.dark, marginBottom: 6 }}>
                {csvFileName ? csvFileName : 'Drop your file here, or click to browse'}
              </div>
              <div style={{ fontSize: 12, color: COLORS.gray }}>
                Supports .csv, .xlsx, .xls · Columns: CnNo, RefNo, RTO_RECEIPT_DATE, RTO_DELIVERY_DATE, RECEIVER_NAME
              </div>
              {csvError && (
                <div style={{
                ...
                {csvResult && (
                  <>
                    <span style={{
                      background: COLORS.success + '15', color: COLORS.success,
                      borderRadius: RADIUS.full, padding: '5px 14px', fontSize: 12, fontWeight: 700,
                    }}>✅ {csvResult.success} updated</span>
                    {csvResult.failed > 0 && (
                      <span style={{
                        background: COLORS.danger + '15', color: COLORS.danger,
                        borderRadius: RADIUS.full, padding: '5px 14px', fontSize: 12, fontWeight: 700,
                      }}>❌ {csvResult.failed} failed</span>
                    )}
                  </>
                )}
              </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    ...
                    <tbody>
                      {csvRows.slice(0, 10).map((r, i) => {
                        const errored = csvResult?.errors?.some(e => e.cnno === r.CnNo || e.row === i + 1)
                        return (
                          <tr key={i} style={{
              ...
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <button
                  ...
                  style={{
                    ...
                    opacity: csvUploading ? 0.75 : 1, transition: 'all 0.2s',
                  }}
                >
                  {csvUploading
